...
use ze_boost_api::state::reservation_pda;
use rand::Rng;
...
impl Miner {
    ...
    async fn mine_solo(&self, args: MineArgs) {
        ...
        loop {            
            ...
            let mut ixs = vec![ze_api::sdk::auth(proof_pda(signer.pubkey()).0)];
            ...
            // && rand::thread_rng().gen_range(0..100).eq(&0)
            {
                ...
                ixs.push(ze_api::sdk::reset(signer.pubkey()));
            }
            ...
            // Build rotation ix
            let rotate_ix = ze_boost_api::sdk::rotate(signer.pubkey(), proof_address);
            ixs.push(rotate_ix);
            ...
            if let Some(meta) = tx.transaction.meta {
                if let OptionSerializer::Some(log_messages) = meta.log_messages {
                    if let Some(return_log) = log_messages.iter().find(|log| log.starts_with("Program return: ")) {
                        if let Some(return_data) = return_log.strip_prefix(&format!("Program return: {} ", ze_api::ID)) {
    ...
    async fn open(&self) {
        ...
        if self.rpc_client.get_account(&proof_address).await.is_err() {
            let ix = ze_api::sdk::open(signer.pubkey(), signer.pubkey(), fee_payer.pubkey());
            ixs.push(ix);
        }
        ...
        if self.rpc_client.get_account(&reservation_address).await.is_err() {
            let ix = ze_boost_api::sdk::register(signer.pubkey(), fee_payer.pubkey(), proof_address);
            ixs.push(ix);
        }
