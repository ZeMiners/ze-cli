...
impl Miner {
        ...
        if let Ok(proof) = proof {
            ...
            data.push(TableData {
                ...
                value: if proof.balance > 0 {
                    format!("{} ZE", amount_u64_to_f64(proof.balance)).bold().yellow().to_string()
                } else {
                    format!("{} ZE", amount_u64_to_f64(proof.balance))
                }
            });
            data.push(TableData {
            ...
            data.push(TableData {
                ...
                value: format!("{} ZE", amount_to_ui_amount(proof.total_rewards, ze_api::consts::TOKEN_DECIMALS)),
            });
            data.push(TableData {
                key: "Miner".to_string(),
    ...
    async fn close(&self, _args: AccountCloseArgs) {
        ...
        if !ask_confirm(
            format!("{} You have {} ZE staked in this account.\nAre you sure you want to {}close this account? [Y/n]", 
                "WARNING".yellow(),
                amount_to_ui_amount(proof.balance, ze_api::consts::TOKEN_DECIMALS),
        ...
        let ix = ze_api::sdk::close(signer.pubkey());
