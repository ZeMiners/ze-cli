...
use colored::*;
use ze_api::state::proof_pda;
use ze_boost_api::{state::{boost_pda, stake_pda, Boost}, consts::BOOST_DENOMINATOR};
...
impl Miner {
    ...
    async fn stake_claim(&self, claim_args: StakeClaimArgs, stake_args: StakeArgs) -> Result<(), Error> {
        ...
        ixs.push(ze_boost_api::sdk::claim(
            pubkey,
            beneficiary,
            mint_address,
            claim_args.amount
        ...
        if let Ok(stake) = stake {
            ...
            data.push(TableData {
                ...
                value: if stake.rewards > 0 {
                    format!("{} ZE", amount_u64_to_f64(stake.rewards)).yellow().bold().to_string()
                } else {
                    format!("{} ZE", amount_u64_to_f64(stake.rewards))
                },
            });
        } else {
            data.push(TableData {
    ...
    async fn fetch_boost_data(&self, address: Pubkey, boost: Boost, mint: Mint, symbol: String, data: &mut Vec<TableData>) {
        ...
        let boost_proof: ze_api::prelude::Proof = get_proof(&self.rpc_client, boost_proof_address).await.expect("Failed to fetch proof account");
        data.push(TableData {
            key: "Address".to_string(),
            value: address.to_string(),
        });
        ...
        data.push(TableData {
            ...
            value: format!(
                "{} ZE",
                amount_to_ui_amount(boost_proof.balance, ze_api::consts::TOKEN_DECIMALS)
            ),
        });
        data.push(TableData {
        ...
        for (address, boost) in boosts {
            ...
            data.push(StakeTableData {
                ...
                my_yield: if stake_rewards > 0 {
                    format!("{} ZE", amount_u64_to_f64(stake_rewards)).yellow().bold().to_string()
                } else {
                    format!("{} ZE", amount_u64_to_f64(stake_rewards))
                },
            });
        }
    ...
    async fn stake_deposit(
        ...
        if self.rpc_client.get_account_data(&stake_address).await.is_err() {
            ...
            let ix = ze_boost_api::sdk::open(signer.pubkey(), signer.pubkey(), mint_address);
            self.send_and_confirm(&[ix], ComputeBudget::Fixed(50_000), false).await.ok();
        }

        // Send tx
        println!("Depositing stake...");
        let ix = ze_boost_api::sdk::deposit(signer.pubkey(), mint_address, amount);
        self.send_and_confirm(&[ix], ComputeBudget::Fixed(50_000), false).await.ok();
        Ok(())
    }
    ...
    async fn stake_withdraw(
        ...
        // Send tx
        ixs.push(ze_boost_api::sdk::withdraw(signer.pubkey(), mint_address, amount));
...
impl Miner {
    async fn stake_migrate(&self, _args: StakeMigrateArgs, _stake_args: StakeArgs) -> Result<(), Error> {
        ...
        for mint in boost_mints {
            let legacy_boost_address = ze_boost_legacy_api::state::boost_pda(mint).0;
            let legacy_stake_address = ze_boost_legacy_api::state::stake_pda(pubkey, legacy_boost_address).0;
            ...
            if let Ok(legacy_stake_account) = get_legacy_stake(&self.rpc_client, legacy_stake_address).await {
                if legacy_stake_account.balance > 0 {
                    if ask_confirm(
                        ...
                        ixs.push(ze_boost_legacy_api::sdk::withdraw(pubkey, mint, legacy_stake_account.balance));
                        if self.rpc_client.get_account_data(&new_stake_address).await.is_err() {
                            ixs.push(ze_boost_api::sdk::open(pubkey, pubkey, mint));
                        }
                        ixs.push(ze_boost_api::sdk::deposit(pubkey, mint, legacy_stake_account.balance));
        ...
        for (pool_address, pool) in pools {
            ...
            for mint in boost_mints {
                ...
                let share_address = ze_pool_api::state::share_pda(signer.pubkey(), pool_address, mint).0;
                if let Ok(share) = get_share(&self.rpc_client, share_address).await {
                    ...
                    if share.balance > 0 {
                        if ask_confirm(
                            ...
                            ixs.push(ze_pool_api::sdk::unstake(signer.pubkey(), mint, pool_address, beneficiary_address, share.balance));
                            if self.rpc_client.get_account_data(&new_stake_address).await.is_err() {
                                ixs.push(ze_boost_api::sdk::open(signer.pubkey(), signer.pubkey(), mint));
                            }
                            ixs.push(ze_boost_api::sdk::deposit(signer.pubkey(), mint, share.balance));
                            println!("Migrating stake...");
                            self.send_and_confirm(&ixs, ComputeBudget::Fixed(400_000), false).await.ok();
                            println!("");
                        }
    ...
    async fn stake_accounts(&self, _args: StakeAccountsArgs, stake_args: StakeArgs) -> Result<(), Error> {
        ...
        for (_stake_address, stake) in stake_accounts {
            data.push(StakerTableData {
                ...
                rewards: format!("{:#.11} ZE", amount_to_ui_amount(stake.rewards, ze_api::consts::TOKEN_DECIMALS)),
            });
        }
        let mut table = Table::new(data);
        table.with(Style::blank());
