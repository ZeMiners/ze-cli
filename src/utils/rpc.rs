...
use ze_boost_api::state::{Boost, Reservation, Stake};
use ze_pool_api::state::{Pool, Member, Share};
...
#[cfg(feature = "admin")]
use ze_boost_api::state::Checkpoint;
...
pub async fn get_boosts(client: &RpcClient) -> Result<Vec<(Pubkey, Boost)>, anyhow::Error> {
    get_program_accounts::<Boost>(client, ze_boost_api::ID, vec![]).await
}

pub async fn get_pools(client: &RpcClient) -> Result<Vec<(Pubkey, Pool)>, anyhow::Error> {
    get_program_accounts::<Pool>(client, ze_pool_api::ID, vec![]).await
}
...
pub async fn get_legacy_stake(client: &RpcClient, address: Pubkey) -> Result<ze_boost_legacy_api::state::Stake, anyhow::Error> {
    let data = client
        .get_account_data(&address)
        .await?;
    Ok(*ze_boost_legacy_api::state::Stake::try_from_bytes(&data)?)
}
...
pub async fn get_boost_stake_accounts(
    ...
    get_program_accounts::<Stake>(rpc_client, ze_boost_api::ID, vec![filter]).await
}
